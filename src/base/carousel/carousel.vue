<template>
  <div class="carousel-wrap" :needCarousel="needCarousel" :needAuto="needAuto" ref="carouselWrap"
    @touchstart.prevent="touchStart"
    @touchmove.prevent="touchMove"
    @touchend.prevent="touchEnd"
  >
    <ul class="list" :style="{width:`${imgs.length}00%`}" ref="ulNode">
      <li v-for="(item,index) in imgs" :key="index" :style="{width:`${1/imgs.length*100}%`}">
        <a :href="item.linkUrl">
          <img :src="item.picUrl" alt="" ref="imgNodes" class="needsclick" @load="loadImage(index)">
        </a>
      </li>
    </ul>
    <div class="points-wrap" ref="pointsWrap">
      <span :class="{active: currentIndex === index }" v-for="(item,index) in datas"></span>
    </div>
  </div>
</template>
    ...
    methods: {
      touchStart(ev){
        ...
        if (this.needCarousel) {
          let index = this.css(this.$refs.ulNode, "translateX") / document.documentElement.clientWidth;
          if (index === 0) {
            index = -this.datas.length;
            this.css(this.$refs.ulNode, "translateX", index * document.documentElement.clientWidth)
      ...
      auto() {
        ...
        this.timer = setInterval(() => {
          ...
          setTimeout(() => {//开启延时是因为js执行很快，前面设置的ulNode.style.transition="none"可能会被覆盖
            index--;
      ...
      loadImage(index) {
        if(!this.checkloaded) {
          ...
          this.$refs.carouselWrap.style.height = this.$refs.imgNodes[index].offsetHeight + "px";
